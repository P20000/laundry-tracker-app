...
const sendViaMailtrapApi = async (options: IMailtrapSendOptions): Promise<boolean> => {
    ...
    try {
        ...
        if (response.ok && data.success) {
            console.log(`✅ Mailtrap Email successfully delivered to ${options.to}! Message IDs:`, data.message_ids);
            return true;
        } else {
            console.error(`❌ Mailtrap API error for ${options.to}:`, data);
            return false;
        }
    } catch (err: any) {
        console.error(`❌ Mailtrap fetch error for ${options.to}:`, err?.message || err);
        return false;
    }
};
